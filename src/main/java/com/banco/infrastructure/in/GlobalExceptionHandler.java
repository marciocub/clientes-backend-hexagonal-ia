...
import com.banco.domain.exception.CuentaNoEncontradoException;
...
import com.banco.domain.exception.NumeroCuentaDuplicadoException;
...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler(LimiteTarjetasExcedidoException.class)
    public ResponseEntity<ErrorRespuesta> manejarLimiteTarjetasExcedido(LimiteTarjetasExcedidoException ex) {
        return ResponseEntity.status(HttpStatus.CONFLICT)
                .body(new ErrorRespuesta(409, "Conflict", ex.getMessage(), LocalDateTime.now()));
    }

    @ExceptionHandler(ConsumoExcedeDisponibleException.class)
    public ResponseEntity<ErrorRespuesta> manejarConsumoExcedeDisponible(ConsumoExcedeDisponibleException ex) {
        return ResponseEntity.status(HttpStatus.CONFLICT)
                .body(new ErrorRespuesta(409, "Conflict", ex.getMessage(), LocalDateTime.now()));
    }

    @ExceptionHandler(FacturaYaPagadaException.class)
    public ResponseEntity<ErrorRespuesta> manejarFacturaYaPagada(FacturaYaPagadaException ex) {
        return ResponseEntity.status(HttpStatus.CONFLICT)
                .body(new ErrorRespuesta(409, "Conflict", ex.getMessage(), LocalDateTime.now()));
    }
